...
uses System.SysUtils, System.Classes, System.Json,
    ...
    untClassLog, ppEndUsr, ppParameter, ppDesignLayer,
    ppBands, ppPrnabl, ppClass, ppCtrls, ppCache,
...
begin
  try
    try
      ...
      gravaStringStream(TConfiguracoesAplicativo.Instancia.arquivodisplay, FAcouguePadaria.AsJson);
      ...
      gravaStringStream(TConfiguracoesAplicativo.Instancia.arquivoaplicativo, FSenhas.AsJson);
...
begin
  try
    ...
    if FileExists(TConfiguracoesAplicativo.Instancia.arquivopadaria) then
      begin
        gravaLog('DISPLAY',
        ...
        gravaStringStream(TConfiguracoesAplicativo.Instancia.arquivopadaria, FMonitorPadaria.AsJson);
      end;
...
function TServerMethods1.chamarnovamente: TJSONObject;
  var
    strJson : String;
    FMonChamarNovamente : TClassMonitor;
begin
  try
    try
      TGeraLog.Instancia.TryGeraLog('GET CHAMAR NOVAMENTE', 'CHAMANDO SENHA NOVAMENTE - INICIO');
      FMonChamarNovamente := TClassMonitor.Create;

      leStringStrream(TConfiguracoesAplicativo.Instancia.arquivodisplay, strJson);
      FMonChamarNovamente.AsJson := strJson;

      FMonChamarNovamente.chamandonovamente := 'sim';
      ...
      gravaStringStream(TConfiguracoesAplicativo.Instancia.arquivopadaria, FMonChamarNovamente.AsJson);
      gravaStringStream(TConfiguracoesAplicativo.Instancia.arquivodisplay, FMonChamarNovamente.AsJson);

      TGeraLog.Instancia.TryGeraLog('GET CHAMAR NOVAMENTE', 'CHAMANDO SENHA NOVAMENTE - FIM');
    except
      // silenciosa
    end;
  finally
    FreeAndNil(FMonChamarNovamente);
  end;
end;
...
begin
  try
    ...
    gravaStringStream(TConfiguracoesAplicativo.Instancia.arquivovisual, FMonVisual.AsJson);
  ...
  try
    try
      ...
      if aJson.Values['tipo'].Value = 'TECLADO DISPLAY 1' then
        ...
        begin
          gravaStringStream(TConfiguracoesAplicativo.Instancia.arquivoacougue, FMonitorPadaria.AsJson);
        ...
        begin
          ...
          gravaStringStream(TConfiguracoesAplicativo.Instancia.arquivoDisplay, aJson.ToString);
          gravaStringStream(TConfiguracoesAplicativo.Instancia.arquivoPadaria, aJson.ToString);
          gravaStringStream(TConfiguracoesAplicativo.Instancia.arquivoacougue, aJson.ToString);
...
begin
  try
    ...
    TConfiguracoesAplicativo.Instancia.arquivosom := aJson.Values['arquivoSom'].value;
    gravaStringStream(TConfiguracoesAplicativo.Instancia.arquivoaplicativo, FAplicativo.AsJson);
...
begin
  ...
  try
    ...
    FMonVisual.Configuracoes.senhaDupla := FMonTemp.Configuracoes.senhaDupla;
    FMonVisual.Configuracoes.doisDisplay := FMonTemp.Configuracoes.doisDisplay;
...
begin
  try
    TGeraLog.Instancia.TryGeraLog('POST DISPLAY', 'SALVANDO CONFIGURACOES DE DISPLAY - INICIO');
    ...
    try
      ...
      gravaStringStream(TConfiguracoesAplicativo.Instancia.arquivodisplay, FMonConf.AsJson);
      ...
      aValue := TJSONObject.ParseJSONValue(GetInvocationMetadata().ResponseContent);
      Result := (aValue as TJSONObject);
      TGeraLog.Instancia.TryGeraLog('POST DISPLAY', 'SALVANDO CONFIGURACOES DE DISPLAY - SUCESSO');
    except
      // silenciosa
    end;
  finally
    ...
    FreeAndNil(FMonConf);
    TGeraLog.Instancia.TryGeraLog('POST DISPLAY', 'SALVANDO CONFIGURACOES DE DISPLAY - FIM');
  end;
end;
